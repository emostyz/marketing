-- AI Slide Generation System Database Schema (CORRECTED VERSION)
-- Migration for comprehensive slide generation pipeline
-- This version properly matches the existing presentations table UUID schema
...
-- Create final presentations table extensions (matching existing UUID id type from schema.ts)
-- This correctly references the presentations table with UUID id
...
-- Create presentation drafts table for version history (matching existing UUID id type)
...
-- Create QA results table (matching existing UUID presentation id type)
...
DROP TRIGGER IF EXISTS update_ai_analysis_results_updated_at ON ai_analysis_results;
CREATE TRIGGER update_ai_analysis_results_updated_at BEFORE UPDATE ON ai_analysis_results
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_slide_structures_updated_at ON slide_structures;
CREATE TRIGGER update_slide_structures_updated_at BEFORE UPDATE ON slide_structures
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_chart_specifications_updated_at ON chart_specifications;
CREATE TRIGGER update_chart_specifications_updated_at BEFORE UPDATE ON chart_specifications
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_presentation_drafts_updated_at ON presentation_drafts;
CREATE TRIGGER update_presentation_drafts_updated_at BEFORE UPDATE ON presentation_drafts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_ai_processing_queue_updated_at ON ai_processing_queue;
