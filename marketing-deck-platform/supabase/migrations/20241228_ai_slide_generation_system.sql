-- AI Slide Generation System Database Schema
...
ALTER TABLE presentations ADD COLUMN IF NOT EXISTS source_dataset_id TEXT REFERENCES datasets(id) ON DELETE SET NULL;
...
-- This is useful for QA testing
/*
INSERT INTO datasets (id, user_id, filename, file_type, size_bytes, rows_count, columns_data, schema_data, preview_data, metadata) VALUES
    ('test_dataset_1', auth.uid(), 'sample_sales_data.csv', 'csv', 15240, 100, 
     '["date", "product", "sales", "region", "customer_type"]',
     '{"date": "date", "product": "categorical", "sales": "number", "region": "categorical", "customer_type": "categorical"}',
     '[{"date": "2024-01-01", "product": "Product A", "sales": 1000, "region": "North", "customer_type": "Enterprise"}]',
     '{"hasHeaders": true, "encoding": "utf-8", "delimiter": ",", "numericColumns": ["sales"], "categoricalColumns": ["product", "region", "customer_type"], "dateColumns": ["date"]}'
    );
*/
