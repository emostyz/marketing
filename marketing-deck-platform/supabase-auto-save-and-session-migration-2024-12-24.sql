-- AUTO-SAVE AND SESSION PERSISTENCE MIGRATION
-- Migration: supabase-auto-save-and-session-migration-2024-12-24.sql
-- Created: 2024-12-24 22:15:00 UTC
...
VALUES ('auto-save-and-session-migration-2024-12-24', 'Complete auto-save system with session storage for intake forms and presentations')
...
-- Add missing columns to presentations table for complete auto-save support
DO $$ 
BEGIN
  ...
  IF EXISTS (
    ...
    -- If user_id exists but is not UUID, we need to convert it
...
SELECT 
  ...
  p.auto_save_count,
  p.is_public,
  COALESCE(array_length(p.slides::json::text[]::text[], 1), 0) as slide_count,
  COALESCE(octet_length(p.slides::text), 0) as estimated_size_bytes,
...
GROUP BY p.id, p.user_id, p.title, p.description, p.status, p.created_at, p.updated_at, 
         p.auto_save_enabled, p.last_auto_saved, p.auto_save_count, p.is_public, p.slides;
...
BEGIN
  RAISE NOTICE '============================================';
  RAISE NOTICE '✅ AUTO-SAVE & SESSION MIGRATION COMPLETED!';
  RAISE NOTICE '============================================';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '   • Real-time presentation auto-save';
  RAISE NOTICE '   • Intake form session persistence';
  ...
  RAISE NOTICE '   2. Set up cron jobs for data cleanup';
