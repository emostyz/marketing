...
async function updateLogosAndHeadersAcrossAllPages(settings: Record<string, any>): Promise<string[]> {
  ...
  const siteName = settings['general.site_name'] || 'EasyDecks.ai'
  ...
  for (const filePath of [...new Set(allFiles)]) {
    try {
      ...
      // Replace EasyDecks.ai references (case-sensitive)
      if (content.includes('EasyDecks.ai') && siteName !== 'EasyDecks.ai') {
        content = content.replace(/EasyDecks.ai/g, siteName)
        modified = true
      }

      // Replace common logo text patterns
      const logoPatterns = [
        { old: />\s*EasyDecks.ai\s*</g, new: `>${siteName}<` },
        { old: /"EasyDecks.ai"/g, new: `"${siteName}"` },
        { old: /'EasyDecks.ai'/g, new: `'${siteName}'` },
        { old: /title="EasyDecks.ai"/g, new: `title="${siteName}"` },
        { old: /alt="EasyDecks.ai"/g, new: `alt="${siteName}"` },
        { old: /aria-label="EasyDecks.ai"/g, new: `aria-label="${siteName}"` }
      ]
      ...
      if (filePath.includes('components') && (filePath.includes('header') || filePath.includes('nav') || filePath.includes('Header') || filePath.includes('Nav'))) {
        const headerPatterns = [
          { old: /className="[^"]*">EasyDecks.ai</g, new: `">${siteName}` },
          { old: /<h1[^>]*>EasyDecks.ai<\/h1>/g, new: siteName }
        ]

        for (const pattern of headerPatterns) {
          if (content.match(pattern.old)) {
            // More careful replacement for header patterns
            content = content.replace(/(<h1[^>]*>)EasyDecks.ai(<\/h1>)/g, `$1${siteName}$2`)
            content = content.replace(/(className="[^"]*">)EasyDecks.ai(<)/g, `$1${siteName}$2`)
            modified = true
          }
        }
      }
...
🎯 Auto-generated by EasyDecks.ai Admin System`
