...
async function generateIntelligentSlideContent(
  ...
  console.log(`🧠 Generating intelligent content for: ${slideStructure.title}`)
  ...
  try {
    ...
    const prompt = `You are a BigBrain Analytics™ senior consultant creating slide content for an executive presentation.
...
Return content as an array of strings suitable for presentation slides.`
    ...
    const response = await Promise.race([
      openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert BigBrain Analytics™ consultant creating executive presentation content. Always provide specific, actionable insights with real business value."
          },
          {
            role: "user",
            content: prompt
    ...
    const structuredContent = lines.map(line => line.replace(/^[•\-\*]\s*/, '').trim()).filter(line => line)

    console.log(`✅ Generated ${structuredContent.length} content items for slide: ${slideStructure.title}`)
    console.log(`📝 Content preview: ${structuredContent.slice(0, 2).join(' | ')}`)
    return structuredContent

  } catch (error) {
    console.error(`❌ OpenAI content generation failed for ${slideStructure.title}:`, error)
  ...
  switch (slideStructure.type) {
    ...
    case 'recommendations':
      content.push(
        ...
        '• Optimize operational efficiency through data-driven improvements',
        '• Implement targeted growth initiatives in high-potential areas',
...
export async function POST(request: NextRequest) {
  ...
  return await Promise.race([
    ...
    console.error('❌ World-class generation timed out or failed:', error)
...
async function generateWorldClassDeck(request: NextRequest) {
  try {
    ...
    console.log('🚀 Starting world-class deck generation...', { 
      datasetId, 
    ...
    if (datasetId.startsWith('demo-')) {
      ...
      console.log('📊 Using demo dataset with', dataset.processed_data.length, 'rows')
    } else {
    ...
    if (context.approvedInsights && context.approvedInsights.length > 0) {
      console.log('✅ Using user-approved insights:', context.approvedInsights.length)
      worldClassInsights = context.approvedInsights
    } else {
      console.log('🔄 Generating new insights (no user approval data)')
    ...
    if (context.deckStructure && context.userCustomizedSlides) {
      console.log('🎯 Using AI-generated user-customized deck structure')
      ...
      const intelligentSlides = await Promise.all(
        context.deckStructure.slides
          ...
          .map(async (slideStructure: any, index: number) => {
            console.log(`🧠 Creating AI slide: ${slideStructure.title}`)
      ...
      console.log('🔄 Creating standard deck structure (fallback)')
      deck = await createDeckStructure(template, worldClassInsights, dataset, context)
    }
    console.log('🏗️ Deck structure created:', { slides: deck.slides.length, aiStructured: deck.aiStructured })
    ...
    console.log('🎨 Visual excellence applied with advanced styling')
    
    // 6.5. Use AI-Powered Slide Builder with advanced editor features
    console.log('🎨 Initializing AI-Powered Slide Builder with advanced editor integration...')
    const aiSlideBuilder = new AIPoweredSlideBuilder()
    
    const aiEnhancedSlides = []
    for (const [index, slide] of polishedSlides.entries()) {
      console.log(`🎯 Building slide ${index + 1} with AI editor features...`)
      ...
      try {
        ...
        console.log(`✅ Slide ${index + 1} AI elements:`, {
        ...
        if (slideResult.elements && slideResult.elements.length > 0) {
          console.log(`📝 First element content preview:`, 
        ...
        console.error(`❌ Error building slide ${index + 1}:`, error)
    ...
    console.log('✨ AI-Powered slides with advanced editor features generated')
    ...
    if (isDemo) {
      ...
      try {
        ...
        if (saveResult.success) {
          ...
          console.log('✅ Enhanced deck saved:', savedDeck.id)
        } else {
          throw new Error(saveResult.error)
        }
      } catch (enhancedError) {
        console.warn('⚠️ Enhanced save failed, using fallback:', enhancedError)
        ...
        console.log('💾 Fallback deck saved:', savedDeck.id)
      }
    ...
    console.log(`🎯 Final deck details:`)
  ...
  switch (slideTemplate.type) {
    ...
    case 'deepDive':
      ...
      if (topInsight) {
        ...
        slide.content = [
          `• ${topInsight.impact || topInsight.businessImplication || 'High-impact business insight'}`,
          ...((topInsight.evidence || topInsight.dataPoints || []).slice(0, 3).map((evidence: string) => `• ${evidence}`)),
          `• Confidence Level: ${topInsight.confidence || 85}%`,
          `• Business Impact: ${topInsight.businessImpact || 'Significant'}`
        ]
...
function generateSlideElements(slide: any, slideTemplate: any, design?: any): any[] {
  ...
  slide.content?.forEach((item: string, index: number) => {
    const isKeyPoint = item.includes('•') || item.match(/^\d+\./);
...
async function createWorldClassSlide(supabase: any, deckId: string, slideData: any, index: number) {
  try {
    ...
    console.log(`✅ Created world-class slide ${index + 1} with ${slideData.elements?.length || 0} sophisticated elements`)
